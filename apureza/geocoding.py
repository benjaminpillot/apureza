...
class SinanParser(XParser):
    ...
    def __init__(self):
        super().__init__()
        self._re_parser.update(casa=r"(\b(?:k(?=sa)|c(?=a|s))a?s?a?\.*\s*[\W_]*)(\d{1,3})?",
                               lote=r"(\blo?t?e?\.*\s*[\W_]*)(\d{1,3})",
                               apartamento=r"(\bapa?r?t?a?m?e?n?t?o?\.*\s*)(\d{1,3})")
...
class BrasiliaParser(SinanParser):
    ...
    def __init__(self):
        ...
        self._re_parser.update(super_quadra_norte=r"(\bsqn\.*\s*)(\d{1,3})",
                               ...
                               chacara=r"(\bcha?c?a?r?a?\.*\s*)((?:\d{1,3})|(?:.+?(?=\d)))")
